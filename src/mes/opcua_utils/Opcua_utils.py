...
def load_tools(client, conn):
    ...
    requires_machine_tools = [machine[1] for machine in machines]
    
    #requires_machine_tools[4] = 1
    #requires_machine_tools[5] = 1
    
    for req_tool, tool_node in zip(requires_machine_tools, Set_Tool_Mx_n):
        setValueCheck(tool_node, req_tool, ua.VariantType.Int16)
    
    
    
    
    Current_tools = [node.get_value() for node in Set_Tool_Mx_n]
    
    
    print("Machine tools: ", requires_machine_tools)
